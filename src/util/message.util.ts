...
import { exactMatch, looseMatch, matchOptions } from "./string.util";
...
export namespace Filter {
    ...
    export function multipleFilters(filters: multipleFiltersInput): AsyncCollectorFilter {
        return async (m: Message) => {
            return new Promise(async (resolve, reject) => {
                ...
                if (!filters.every || filters.every.length == 0) { everyRes = true }
                else {
                ...
                if (!filters.some || filters.some.length == 0) { someRes = true } else {
    ...
    // both are inclusive
    export function numberRangeFilter(min: number, max: number, multiple = false): CollectorFilter {
        return (response: Message) => {
            const validNumber = function (content: string): boolean {
                const num_resp = parseInt(content);
                return num_resp != NaN && (num_resp <= max) && num_resp >= min
            }

            const trimmedContent = response.content.trim();
            return multiple ?
                trimmedContent.split(' ').every(x => validNumber(x)) : validNumber(trimmedContent)
        }
    }
    ...
    export function validUserFilter(guild: Guild): CollectorFilter {
        return (response: Message) => response.content.trim().split(' ').some(x => guild.member(x))
    }

    export function validChannelFilter(guild: Guild): CollectorFilter {
        return (response: Message) =>
            response.content.trim().split(" ").some(x => guild.channels.resolve(x))
    }

    export function stringFilter(str: string, options: matchOptions): CollectorFilter {
        const regex = options.loose ? looseMatch(str, options) : exactMatch(str, options);
...
export function sendToChannel(channel: MessageChannel, statement: string | MessageEmbed, autoDelete = true, autoDeleteTime = 15 * NumberConstants.secs): Promise<sendUtilResponse> {
    return sendUtil(channel.send(statement), autoDelete, autoDeleteTime);
}

export function replyUtil(message: Message, statement: string | MessageEmbed, autoDelete: boolean, time = 15 * NumberConstants.secs): Promise<sendUtilResponse> {
...
export namespace Prompt {
    ...
    export function getSameUserInput(userID: string, textChannel: MessageChannel, messagePrompt: string | MessageEmbed, filter: AnyCollectorFilter, time = 30 * NumberConstants.secs): Promise<Message> {
        ...
        return new Promise((resolve, reject) => {
            ...
            textChannel.send(messagePrompt).then((promptMessage: Message) => {
                ...
                messageCollector.on('collect', async (m: Message) => {
                    ...
                    if (completed) deleteMessage(promptMessage, 10 * NumberConstants.secs).catch((error) => { console.error(error) });
                ...
                messageCollector.on('end', (collected, reason) => {
                    ...
                    if (lastMessage == undefined) { reject(undefined); return }

                    if ([quit, messageCollectorTimeout].includes(reason)) {
                        const msg: string = reason == messageCollectorTimeout ? 'No reponse. Quitting...' : 'Quitting...'
                        sendToChannel(textChannel, msg, true, 10 * NumberConstants.secs).then(() => {
                            reject(undefined)
                        }).catch((error) => {
                            reject(error)
                        });
    ...
    export function getMultipleInput(userID: string, textChannel: MessageChannel, filter: AsyncCollectorFilter | CollectorFilter, numberOfInputs: number, unique = true): Promise<string[]> {

        return new Promise(async (resolve, reject) => {

            const inputs: string[] = [];

            const fullFilter = !unique ? filter :
                Filter.multipleFilters({ every: [filter], some: [Filter.uniqueInputFilter(inputs)] })
     ...
     */
    export function optionSelect(userID: string, textChannel: MessageChannel, functionArray: optionSelectElement[], options?: optionSelectOptions): Promise<any> {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                optionObj.function(optionObj.args).then((x: any) => {
                    resolve(x);
                }).catch((error: any) => {
                    reject(error)
                });
    ...
    export function arraySelect<T>(userID: string, textChannel: MessageChannel, array: Array<T>, message: MessageEmbed, options?: arraySelectOptions): Promise<T | T[]> {
        const time = options?.time ? options.time : 15 * NumberConstants.mins;
        const offset = options?.customOffset ? options.customOffset : 1
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                const userSelectionMsg = await getSameUserInput(
                    ...
                    textChannel,
                    message,
                    Filter.numberRangeFilter(offset, array.length, true),
                    time
                );

                if (options?.multiple) {
                    const elementNumbers: number[] = userSelectionMsg.content.trim().split(/[ ,]+/).map(x => (parseInt(x) - offset));
