...
import { selfPronouns, groupPronouns, NumberConstants } from '../util/constants'
...
import { IConfigDoc, ILinkDoc, IAutoDeletePrefix } from '../db/models/guild.model'
...
export namespace ConfigUtil {
    ...
    export async function setPrefixAutoDelete(args: ConfigUtilFunctionArgs) {
        try {
            const options: Prompt.optionSelectElement[] = [
                ...
                { name: 'Edit exiting prefix settings', function: editPrefixAutoDelete, args: args },
    ...
    export async function addPrefixAutoDelete(args: ConfigUtilFunctionArgs) {
        try {
            ...
            const foundPrefix = config.autodelete_prefixes.find((x) => x.prefix === prefix)
            if (!foundPrefix) {
                // new prefix, add and then go to the edit move
                await Config.addPrefixAutoDelete(args.guildID, prefix, args.textChannel)
            }
            args.prefix = prefix
    ...
    export async function selectPrefix(args: ConfigUtilFunctionArgs) {
        try {
            ...
            const prompt = `Select a prefix to edit:\n ${autodeletePrefixes
                ...
                .join('\n')}`
            const prefix = await Prompt.arraySelect(args.userID, args.textChannel, autodeletePrefixes, prompt, {
                multiple: false,
                customOffset: offset,
            })
    ...
    export async function editPrefixAutoDelete(args: ConfigUtilFunctionArgs) {
        ...
        const options: Prompt.optionSelectElement[] = [
            ...
            {
                name: `Change mode. ${prefixDoc.allowMode ? allowModeString : deleteModeString}`,
    ...
    export async function changeAutoDeleteMode(args: ConfigUtilFunctionArgs) {
        try {
            ...
            await Config.changePrefixAutoDeleteMode(args.guildID, args.prefix, args.textChannel)
    ...
    export async function removePrefixAutoDelete(args: ConfigUtilFunctionArgs) {
        try {
            ...
            if (!args.prefix) {
                ...
                await Config.deletePrefixAutoDelete(args.guildID, args.prefix, args.textChannel)
    ...
    export async function editAllowList(args: ConfigUtilFunctionArgs) {
        try {
            ...
            const prefixDoc = await Config.getAutoDeletePrefix(args.guildID, args.prefix)
            if (!prefixDoc) {
                sendToChannel(args.textChannel, 'Prefix not loaded in for whatever reason. Quitting...')
                return undefined
            }
            ...
            const options: Prompt.optionSelectElement[] = [
                { name: 'Add to allow list', function: () => {}, args: args },
                { name: 'Remove from allow list', function: () => {}, args: args },
