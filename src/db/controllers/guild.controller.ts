		...
		return new Promise((resolve, reject) => {
			...
			Guild.getGuild(guildID)
				.then((guildDoc) => {
					...
					const { approved, rejected } = uniqueArrayObject(
						aliases,
						names,
						"name"
					);
					...
					if (textChannel) {
						...
						if (approved.length > 0)
							msg.addField("Approved Aliases", approved.join("\n"), false);
						if (rejected.length > 0)
							msg.addField("Rejected Aliases", rejected.join("\n"), false);
					...
					if (approved.length == 0) {
						if (textChannel)
							sendToChannel(
								textChannel,
								"No approved aliases. Nothing will be added.",
								true
							);
					...
					Guilds.updateOne({ guild_id: guildID }, query).then(
						(x: updateOneResponse) => {
	...
	export function deleteAliases(
		...
		return new Promise((resolve, reject) => {
			...
			const updateStrings: updateOneStrings = {
				...
				failure:
					"There was an error deleting the aliases. Please try again or contact the administrator",
			};
...
export namespace Link {
		...
		return new Promise((resolve, reject) => {
			Guild.getGuild(guildID)
				.then((guildDoc) => {
					...
					const allUsedNames: string[] = _.flatten(
						links.map((link) => link.names)
					);
					...
					if (textChannel) {
						...
						if (approved.length > 0)
							msg.addField("Approved Names", approved.join("\n"), false);
						if (rejected.length > 0)
							msg.addField("Rejected Names", rejected.join("\n"), false);
	...
	export async function viewLinks(
		guildID: string,
		textChannel?: MessageChannel
	): Promise<viewLinksResponse> {
		try {
			const guildDoc = await Guild.getGuild(guildID);
			...
			const [clips, links] = _.partition(
				allLinks,
				(link: ILinkDoc) => link.type == Link.LinkTypes.clip
			);
				...
				types.forEach((type) => {
					...
					for (i = 0, j = arr.length; i < j; i += chunk) {
						...
						const linkMessage = linkChunk.map((x, k) => {
							return `${"**" + (i + k + offset) + "**. "}[${x.names.join(
								", "
							)}](${x.link} 'Link')`;
						});
		...
		try {
			...
			if (links.length < 1) {
				sendToChannel(
					...
					message + ". Quitting...",
					true,
					15 * NumberConstants.secs
				);
				return undefined;
			}

			const link = await Prompt.arraySelect(
				userID,
				textChannel,
				links,
				message,
	...
	export async function addNames(
		...
		try {
			...
			const allUsedNames: string[] = _.flatten(
				guildDoc.links.map((link) => link.names)
			);
			const { approved, rejected } = uniqueArrayObject(allUsedNames, names);
			...
			if (approved.length < 1) {
				if (textChannel)
					sendToChannel(
						textChannel,
						"All input names are not available.",
						true
					);
				return undefined;
			}
			...
			const updateStrings: updateOneStrings = {
				success: `Name${multiple ? "s" : ""} ${approved.join(", ")} ${
					multiple ? "have" : "has"
				} been added. ${
					rejected.length > 0
						? `Name${multiple ? "s" : ""} ${rejected.join(", ")} ${
								multiple ? "are" : "is"
						  } already in use.`
						: ""
				}`,
	...
	export async function deleteLink(
		...
		try {
			...
			if (Array.isArray(link)) {
				...
				successStr = `Links known as "${link
					.map((x) => x.names[0])
					.join(", ")}" has been deleted.`;
			} else {
				allIDs = link._id;
				successStr = `The link known as "${link.names.join(
					", "
				)}" has been deleted.`;
			}
			...
			const updateStrings: updateOneStrings = {
				success: successStr,
				failure:
					"There was an error deleting the link. Please try again or contact the administrator",
			};
...
export namespace Movie {
	export async function getMovie(
		guildID: string
	): Promise<IMovieContainerDoc | undefined> {
		try {
			const guildDoc = await Guild.getGuild(guildID);
	...
	export function viewMovieParams(
		guildID: string,
		movieNameSearch: string,
		loose = true
	): viewMovieParamsResponse {
	...
	export async function getMovies(
		...
		try {
			...
			if (!movieDoc || movieDoc.movies.length < 1) {
				...
				if (movieName != "")
					message += ` with search parameters "${movieName}"`;
				movies = [];
			} else {
	...
	export async function getMovieDefaultPassword(
		guildID: string
	): Promise<string> {
		try {
			const movieDoc = await getMovie(guildID);
	...
	export async function addMovie(
		...
		try {
			...
			const res = updateOneResponseHandler(
				movieUpdateResponse,
				updateStrings,
				textChannel
			);
	...
	export async function deleteMovie(
		...
		try {
			...
			const updateStrings: updateOneStrings = {
				...
				failure: `There was an error deleting ${deleteNames.join(
					", "
				)} from the catalog `,
			};
			...
			if (textChannel)
				sendToChannel(textChannel, "Error deleting movies", true);
			throw error;
		}
	}
}
...
export namespace Config {
	...
	export async function addPrefixAutoDelete(
		...
		try {
			...
			return await updateOneResponseHandler(
				response,
				{
					success: `${prefix} has been added`,
					failure: "Prefix failed to add",
				},
				textChannel
			);
		} catch (error) {
	...
	export async function getAutoDeletePrefix(
		...
		try {
			...
			const foundPrefix = configDoc.autodelete_prefixes.find(
				(x) => x.prefix === prefix
			);
	...
	export async function changePrefixAutoDeleteMode(
		guildID: string,
		prefixDoc: IAutoDeletePrefixDoc,
		textChannel?: MessageChannel
	) {
		// getting the latest
		...
		const response = await Guilds.updateOne(
			{
				guild_id: guildID,
				"config.autodelete_prefixes.prefix._id": prefixDoc._id,
			},
			{
				$set: {
					"config.autodelete_prefixes.$.allowMode": !prefixDoc.allowMode,
				},
			}
		);
	...
	export async function deletePrefixAutoDelete(
		guildID: string,
		prefix: string,
		textChannel?: MessageChannel
	) {
		try {
			...
			const response = await Guilds.updateOne(
				...
				{
					$pull: {
						"config.autodelete_prefixes": { _id: { $in: [prefixDoc._id] } },
					},
				}
			);
	...
	export async function addPrefixAutoDeleteAllowed(
		guildID: string,
		prefixDoc: IAutoDeletePrefixDoc,
		whitelist: string[],
		textChannel?: MessageChannel
		...
		try {
			...
			const response = await Guilds.updateOne(
				...
				{
					$addToSet: {
						"config.autodelete_prefixes.$.allowList": { $each: whitelist },
					},
				}
			);
	...
	export async function removePrefixAutoDeleteAllowed() {}

	export async function addPrefixAutoDeleteSpecified() {}

	export async function remvovePrefixAutoDeleteSpecified() {}
}
